...
def q2(pa, su, ps, na, re):
    europe = "EUROPE"
    brass = "BRASS"
    region_nation_index = re.sum(lambda x: ({x[0]: x[1]}) if (x[0].r_name == europe) else (None))
    ...
    region_nation_supplier_index = region_nation_0.sum(
        lambda x: {record({"n_nationkey": x[0].n_nationkey, "n_name": x[0].n_name}): True})

    region_nation_supplier_build_nest_dict = region_nation_supplier_index.sum(
    ...
    region_nation_supplier_partsupp_index = region_nation_supplier_0.sum(lambda x: {record(
    ...
    region_nation_supplier_2 = su.sum(
        lambda x: (region_nation_supplier_build_nest_dict[x[0].s_nationkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (region_nation_supplier_build_nest_dict[x[0].s_nationkey] != None) else (None))

    region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_index = region_nation_supplier_2.sum(
        lambda x: {record(
            {"s_suppkey": x[0].s_suppkey, "s_acctbal": x[0].s_acctbal, "s_name": x[0].s_name, "n_name": x[0].n_name,
             "s_address": x[0].s_address, "s_phone": x[0].s_phone, "s_comment": x[0].s_comment}): True})
    ...
    region_nation_supplier_partsupp_build_nest_dict = region_nation_supplier_partsupp_index.sum(
        lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})

    region_nation_supplier_partsupp_0 = ps.sum(lambda x: (
        region_nation_supplier_partsupp_build_nest_dict[x[0].ps_suppkey].sum(lambda y: {x[0].concat(y[0]): True})
        ) if (region_nation_supplier_partsupp_build_nest_dict[x[0].ps_suppkey] != None) else (None))

    region_nation_supplier_partsupp_1 = region_nation_supplier_partsupp_0.sum(
        lambda x: {record({"ps_partkey": x[0].ps_partkey}): record({"ps_supplycost": x[0].ps_supplycost})})

    region_nation_supplier_partsupp_2 = region_nation_supplier_partsupp_1.sum(lambda x: {x[0].concat(x[1]): True})

    region_nation_supplier_partsupp_part_partsupp_index = region_nation_supplier_partsupp_2.sum(
        lambda x: {x[0].concat(record({"min_supplycost": x[0].ps_supplycost})): x[1]})

    part_0 = pa.sum(lambda x: ({x[0]: x[1]}) if (((endsWith(x[0].p_type, brass)) * (x[0].p_size == 15))) else (None))

    part_partsupp_index = part_0.sum(lambda x: {record({"p_partkey": x[0].p_partkey, "p_mfgr": x[0].p_mfgr}): True})

    part_partsupp_build_nest_dict = part_partsupp_index.sum(lambda x: {x[0].p_partkey: sr_dict({x[0]: x[1]})})

    region_nation_supplier_partsupp_part_partsupp_probe = ps.sum(
    ...
    region_nation_supplier_partsupp_part_partsupp_build_nest_dict = region_nation_supplier_partsupp_part_partsupp_index.sum(
        lambda x: {x[0].ps_partkey: sr_dict({x[0]: x[1]})})

    region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_probe = region_nation_supplier_partsupp_part_partsupp_probe.sum(
        lambda x: (region_nation_supplier_partsupp_part_partsupp_build_nest_dict[x[0].ps_partkey].sum(
            lambda y: {x[0].concat(y[0]): True})
                   ) if (region_nation_supplier_partsupp_part_partsupp_build_nest_dict[x[0].ps_partkey] != None) else (
            None))

    region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_build_nest_dict = region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_index.sum(
        lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})

    region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_0 = region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_probe.sum(
        lambda x: (
            region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_build_nest_dict[x[0].ps_suppkey].sum(
                lambda y: {x[0].concat(y[0]): True})
            ) if (region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_build_nest_dict[
                      x[0].ps_suppkey] != None) else (None))

    region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_1 = region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_0.sum(
        lambda x: ({x[0]: x[1]}) if (x[0].ps_supplycost == x[0].min_supplycost) else (None))

    results = region_nation_supplier_region_nation_supplier_partsupp_part_partsupp_1.sum(lambda x: {record(
...
def q3(li, cu, ord):
    building = "BUILDING"
    customer_orders_index = cu.sum(lambda x: ({x[0]: x[1]}) if (x[0].c_mktsegment == building) else (None))

    customer_orders_probe = ord.sum(lambda x: ({x[0]: x[1]}) if (x[0].o_orderdate < 19950315) else (None))

    customer_orders_build_nest_dict = customer_orders_index.sum(lambda x: {x[0].c_custkey: sr_dict({x[0]: x[1]})})

    customer_orders_lineitem_index = customer_orders_probe.sum(
    ...
    customer_orders_lineitem_probe = li.sum(lambda x: ({x[0]: x[1]}) if (x[0].l_shipdate > 19950315) else (None))

    customer_orders_lineitem_build_nest_dict = customer_orders_lineitem_index.sum(
        lambda x: {x[0].o_orderkey: sr_dict({x[0]: x[1]})})

    customer_orders_lineitem_0 = customer_orders_lineitem_probe.sum(
...
def q4(ord, li):
    ...
    lineitem_orders_isin_build_index = lineitem_1.sum(lambda x: {x[0].l_orderkey: True})
    ...
    orders_2 = orders_1.sum(lambda x: {record({"o_orderpriority": x[0].o_orderpriority}): record({"order_count": 1.0})})
...
def q5(li, cu, ord, re, na, su):
    ...
    region_nation_customer_index = na.sum(
        lambda x: (region_nation_build_nest_dict[x[0].n_regionkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (region_nation_build_nest_dict[x[0].n_regionkey] != None) else (None))

    region_nation_customer_build_nest_dict = region_nation_customer_index.sum(
        lambda x: {x[0].n_nationkey: sr_dict({x[0]: x[1]})})

    region_nation_customer_orders_index = cu.sum(
    ...
    region_nation_customer_orders_probe = ord.sum(
        lambda x: ({x[0]: x[1]}) if (((x[0].o_orderdate >= 19940101) * (x[0].o_orderdate < 19950101))) else (None))

    region_nation_customer_orders_build_nest_dict = region_nation_customer_orders_index.sum(
        lambda x: {x[0].c_custkey: sr_dict({x[0]: x[1]})})

    region_nation_customer_orders_lineitem_index = region_nation_customer_orders_probe.sum(lambda x: (
    ...
    region_nation_customer_orders_lineitem_build_nest_dict = region_nation_customer_orders_lineitem_index.sum(
        lambda x: {x[0].o_orderkey: sr_dict({x[0]: x[1]})})

    supplier_region_nation_customer_orders_lineitem_probe = li.sum(lambda x: (
    ...
    supplier_region_nation_customer_orders_lineitem_0 = supplier_region_nation_customer_orders_lineitem_probe.sum(
        lambda x: (supplier_region_nation_customer_orders_lineitem_build_nest_dict[
                       record({"l_suppkey": x[0].l_suppkey, "c_nationkey": x[0].c_nationkey})].sum(
            lambda y: {x[0].concat(y[0]): True})
                   ) if (supplier_region_nation_customer_orders_lineitem_build_nest_dict[
                             record({"l_suppkey": x[0].l_suppkey, "c_nationkey": x[0].c_nationkey})] != None) else (
            None))
...
def q7(su, li, ord, cu, na):
    ...
    nation_supplier_nation_customer_orders_lineitem_index = nation_supplier_0.sum(
        lambda x: {x[0].concat(record({"n1_name": x[0].n_name})): x[1]})

    nation_customer_index = na.sum(
        lambda x: ({x[0]: x[1]}) if (((x[0].n_name == france) + (x[0].n_name == germany))) else (None))

    nation_customer_build_nest_dict = nation_customer_index.sum(lambda x: {x[0].n_nationkey: sr_dict({x[0]: x[1]})})

    nation_customer_orders_index = cu.sum(
        lambda x: (nation_customer_build_nest_dict[x[0].c_nationkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (nation_customer_build_nest_dict[x[0].c_nationkey] != None) else (None))

    nation_customer_orders_build_nest_dict = nation_customer_orders_index.sum(
        lambda x: {x[0].c_custkey: sr_dict({x[0]: x[1]})})

    nation_customer_orders_0 = ord.sum(
        lambda x: (nation_customer_orders_build_nest_dict[x[0].o_custkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (nation_customer_orders_build_nest_dict[x[0].o_custkey] != None) else (None))

    nation_customer_orders_lineitem_index = nation_customer_orders_0.sum(
        lambda x: {x[0].concat(record({"n2_name": x[0].n_name})): x[1]})

    nation_customer_orders_lineitem_probe = li.sum(
        lambda x: ({x[0]: x[1]}) if (((x[0].l_shipdate >= 19950101) * (x[0].l_shipdate <= 19961231))) else (None))

    nation_customer_orders_lineitem_build_nest_dict = nation_customer_orders_lineitem_index.sum(
        lambda x: {x[0].o_orderkey: sr_dict({x[0]: x[1]})})

    nation_supplier_nation_customer_orders_lineitem_probe = nation_customer_orders_lineitem_probe.sum(lambda x: (
    ...
    nation_supplier_nation_customer_orders_lineitem_build_nest_dict = nation_supplier_nation_customer_orders_lineitem_index.sum(
        lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})

    nation_supplier_nation_customer_orders_lineitem_0 = nation_supplier_nation_customer_orders_lineitem_probe.sum(
        lambda x: (nation_supplier_nation_customer_orders_lineitem_build_nest_dict[x[0].l_suppkey].sum(
            lambda y: {x[0].concat(y[0]): True})
                   ) if (nation_supplier_nation_customer_orders_lineitem_build_nest_dict[x[0].l_suppkey] != None) else (
            None))

    nation_supplier_nation_customer_orders_lineitem_1 = nation_supplier_nation_customer_orders_lineitem_0.sum(
        lambda x: ({x[0]: x[1]}) if (((((x[0].n1_name == france) * (x[0].n2_name == germany))) + (
        ((x[0].n1_name == germany) * (x[0].n2_name == france))))) else (None))

    nation_supplier_nation_customer_orders_lineitem_2 = nation_supplier_nation_customer_orders_lineitem_1.sum(
        lambda x: {x[0].concat(record({"supp_nation": x[0].n1_name})): x[1]})

    nation_supplier_nation_customer_orders_lineitem_3 = nation_supplier_nation_customer_orders_lineitem_2.sum(
        lambda x: {x[0].concat(record({"cust_nation": x[0].n2_name})): x[1]})

    nation_supplier_nation_customer_orders_lineitem_4 = nation_supplier_nation_customer_orders_lineitem_3.sum(
        lambda x: {x[0].concat(record({"l_year": extractYear(x[0].l_shipdate)})): x[1]})

    nation_supplier_nation_customer_orders_lineitem_5 = nation_supplier_nation_customer_orders_lineitem_4.sum(
        lambda x: {x[0].concat(record({"volume": ((x[0].l_extendedprice) * (((1.0) - (x[0].l_discount))))})): x[1]})

    nation_supplier_nation_customer_orders_lineitem_6 = nation_supplier_nation_customer_orders_lineitem_5.sum(
        lambda x: {record({"supp_nation": x[0].supp_nation, "cust_nation": x[0].cust_nation, "l_year": x[0].l_year,
                           "volume": x[0].volume}): True})

    nation_supplier_nation_customer_orders_lineitem_7 = nation_supplier_nation_customer_orders_lineitem_6.sum(
        lambda x: {
            record({"supp_nation": x[0].supp_nation, "cust_nation": x[0].cust_nation, "l_year": x[0].l_year}): record(
                {"revenue": x[0].volume})})

    results = nation_supplier_nation_customer_orders_lineitem_7.sum(lambda x: {x[0].concat(x[1]): True})
...
def q8(pa, su, li, ord, cu, na, re):
    ...
    nation_supplier_part_region_nation_customer_orders_lineitem_index = n2_2.sum(
        lambda x: {x[0].concat(record({"n2_comment": x[0].n_comment})): x[1]})

    part_region_nation_customer_orders_lineitem_index = pa.sum(
    ...
    region_nation_customer_index = region_nation_probe.sum(
        lambda x: (region_nation_build_nest_dict[x[0].n1_regionkey].sum(lambda y: {x[0].concat(y[0]): True})
    ...
    part_region_nation_customer_orders_lineitem_probe = li.sum(lambda x: (
        region_nation_customer_orders_lineitem_build_nest_dict[x[0].l_orderkey].sum(lambda y: {x[0].concat(y[0]): True})
        ) if (region_nation_customer_orders_lineitem_build_nest_dict[x[0].l_orderkey] != None) else (None))

    part_region_nation_customer_orders_lineitem_build_nest_dict = part_region_nation_customer_orders_lineitem_index.sum(
        lambda x: {x[0].p_partkey: sr_dict({x[0]: x[1]})})

    supplier_part_region_nation_customer_orders_lineitem_probe = part_region_nation_customer_orders_lineitem_probe.sum(
        lambda x: (part_region_nation_customer_orders_lineitem_build_nest_dict[x[0].l_partkey].sum(
            lambda y: {x[0].concat(y[0]): True})
                   ) if (part_region_nation_customer_orders_lineitem_build_nest_dict[x[0].l_partkey] != None) else (
            None))

    supplier_part_region_nation_customer_orders_lineitem_build_nest_dict = su.sum(
        lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})

    nation_supplier_part_region_nation_customer_orders_lineitem_probe = supplier_part_region_nation_customer_orders_lineitem_probe.sum(
        lambda x: (supplier_part_region_nation_customer_orders_lineitem_build_nest_dict[x[0].l_suppkey].sum(
            lambda y: {x[0].concat(y[0]): True})
                   ) if (
                    supplier_part_region_nation_customer_orders_lineitem_build_nest_dict[x[0].l_suppkey] != None) else (
            None))

    nation_supplier_part_region_nation_customer_orders_lineitem_build_nest_dict = nation_supplier_part_region_nation_customer_orders_lineitem_index.sum(
        lambda x: {x[0].n2_nationkey: sr_dict({x[0]: x[1]})})

    nation_supplier_part_region_nation_customer_orders_lineitem_0 = nation_supplier_part_region_nation_customer_orders_lineitem_probe.sum(
        lambda x: (nation_supplier_part_region_nation_customer_orders_lineitem_build_nest_dict[x[0].s_nationkey].sum(
            lambda y: {x[0].concat(y[0]): True})
                   ) if (nation_supplier_part_region_nation_customer_orders_lineitem_build_nest_dict[
                             x[0].s_nationkey] != None) else (None))

    nation_supplier_part_region_nation_customer_orders_lineitem_1 = nation_supplier_part_region_nation_customer_orders_lineitem_0.sum(
        lambda x: {x[0].concat(record({"o_year": extractYear(x[0].o_orderdate)})): x[1]})

    nation_supplier_part_region_nation_customer_orders_lineitem_2 = nation_supplier_part_region_nation_customer_orders_lineitem_1.sum(
        lambda x: {x[0].concat(record({"volume": ((x[0].l_extendedprice) * (((1.0) - (x[0].l_discount))))})): x[1]})

    nation_supplier_part_region_nation_customer_orders_lineitem_3 = nation_supplier_part_region_nation_customer_orders_lineitem_2.sum(
        lambda x: {x[0].concat(record({"nation": x[0].n2_name})): x[1]})

    nation_supplier_part_region_nation_customer_orders_lineitem_4 = nation_supplier_part_region_nation_customer_orders_lineitem_3.sum(
        lambda x: {record(
            {"o_year": x[0].o_year, "volume": x[0].volume, "nation": x[0].nation, "volume_A": x[0].volume_A}): True})

    nation_supplier_part_region_nation_customer_orders_lineitem_5 = nation_supplier_part_region_nation_customer_orders_lineitem_4.sum(
        lambda x: {x[0].concat(record({"volume_A": (x[0].volume) if (x[0].nation == brazil) else (0.0)})): x[1]})

    nation_supplier_part_region_nation_customer_orders_lineitem_6 = nation_supplier_part_region_nation_customer_orders_lineitem_5.sum(
        lambda x: {record({"o_year": x[0].o_year}): record({"A": x[0].volume_A, "B": x[0].volume})})

    nation_supplier_part_region_nation_customer_orders_lineitem_7 = nation_supplier_part_region_nation_customer_orders_lineitem_6.sum(
        lambda x: {x[0].concat(x[1]): True})

    nation_supplier_part_region_nation_customer_orders_lineitem_8 = nation_supplier_part_region_nation_customer_orders_lineitem_7.sum(
        lambda x: {x[0].concat(record({"mkt_share": ((x[0].A) / (x[0].B))})): x[1]})

    results = nation_supplier_part_region_nation_customer_orders_lineitem_8.sum(
...
def q9(li, ord, na, su, pa, ps):
    ...
    nation_supplier_part_partsupp_index = su.sum(
        lambda x: (nation_supplier_build_nest_dict[x[0].s_nationkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (nation_supplier_build_nest_dict[x[0].s_nationkey] != None) else (None))
    ...
    nation_supplier_part_partsupp_probe = ps.sum(
        lambda x: (part_partsupp_build_nest_dict[x[0].ps_partkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (part_partsupp_build_nest_dict[x[0].ps_partkey] != None) else (None))

    nation_supplier_part_partsupp_build_nest_dict = nation_supplier_part_partsupp_index.sum(
        lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})

    nation_supplier_part_partsupp_orders_lineitem_index = nation_supplier_part_partsupp_probe.sum(lambda x: (
    ...
    nation_supplier_part_partsupp_orders_lineitem_probe = li.sum(
    ...
    nation_supplier_part_partsupp_orders_lineitem_build_nest_dict = nation_supplier_part_partsupp_orders_lineitem_index.sum(
        lambda x: {record({"ps_partkey": x[0].ps_partkey, "ps_suppkey": x[0].ps_suppkey}): sr_dict({x[0]: x[1]})})

    nation_supplier_part_partsupp_orders_lineitem_0 = nation_supplier_part_partsupp_orders_lineitem_probe.sum(
        lambda x: (nation_supplier_part_partsupp_orders_lineitem_build_nest_dict[
                       record({"l_partkey": x[0].l_partkey, "l_suppkey": x[0].l_suppkey})].sum(
            lambda y: {x[0].concat(y[0]): True})
                   ) if (nation_supplier_part_partsupp_orders_lineitem_build_nest_dict[
                             record({"l_partkey": x[0].l_partkey, "l_suppkey": x[0].l_suppkey})] != None) else (None))
    ...
    nation_supplier_part_partsupp_orders_lineitem_4 = nation_supplier_part_partsupp_orders_lineitem_3.sum(
        lambda x: {record({"nation": x[0].nation, "o_year": x[0].o_year, "amount": x[0].amount}): True})

    nation_supplier_part_partsupp_orders_lineitem_5 = nation_supplier_part_partsupp_orders_lineitem_4.sum(
...
def q10(cu, ord, li, na):
    r = "R"
    customer_orders_index = cu.sum(lambda x: {record(
        {"c_custkey": x[0].c_custkey, "c_name": x[0].c_name, "c_acctbal": x[0].c_acctbal, "c_phone": x[0].c_phone,
         "c_address": x[0].c_address, "c_comment": x[0].c_comment, "c_nationkey": x[0].c_nationkey}): True})

    customer_orders_probe = ord.sum(
        lambda x: ({x[0]: x[1]}) if (((x[0].o_orderdate >= 19931001) * (x[0].o_orderdate < 19940101))) else (None))

    customer_orders_build_nest_dict = customer_orders_index.sum(lambda x: {x[0].c_custkey: sr_dict({x[0]: x[1]})})

    nation_customer_orders_probe = customer_orders_probe.sum(
        lambda x: (customer_orders_build_nest_dict[x[0].o_custkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (customer_orders_build_nest_dict[x[0].o_custkey] != None) else (None))

    nation_customer_orders_build_nest_dict = na.sum(lambda x: {x[0].n_nationkey: sr_dict({x[0]: x[1]})})

    nation_customer_orders_0 = nation_customer_orders_probe.sum(
    ...
    nation_customer_orders_lineitem_index = nation_customer_orders_0.sum(lambda x: {record(
    ...
    nation_customer_orders_lineitem_probe = li.sum(lambda x: ({x[0]: x[1]}) if (x[0].l_returnflag == r) else (None))

    nation_customer_orders_lineitem_build_nest_dict = nation_customer_orders_lineitem_index.sum(
        lambda x: {x[0].o_orderkey: sr_dict({x[0]: x[1]})})

    nation_customer_orders_lineitem_0 = nation_customer_orders_lineitem_probe.sum(lambda x: (
        nation_customer_orders_lineitem_build_nest_dict[x[0].l_orderkey].sum(lambda y: {x[0].concat(y[0]): True})
        ) if (nation_customer_orders_lineitem_build_nest_dict[x[0].l_orderkey] != None) else (None))
...
def q11(ps, su, na):
    germany = "GERMANY"
    nation_supplier_index = na.sum(lambda x: ({x[0]: x[1]}) if (x[0].n_name == germany) else (None))

    nation_supplier_build_nest_dict = nation_supplier_index.sum(lambda x: {x[0].n_nationkey: sr_dict({x[0]: x[1]})})

    nation_supplier_partsupp_index = su.sum(
        lambda x: (nation_supplier_build_nest_dict[x[0].s_nationkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (nation_supplier_build_nest_dict[x[0].s_nationkey] != None) else (None))

    nation_supplier_partsupp_build_nest_dict = nation_supplier_partsupp_index.sum(
        lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})
    ...
    tmp_calc_value = nation_supplier_partsupp_0.sum(
        lambda x: ((((x[0].ps_supplycost) * (x[0].ps_availqty))) * (0.0001)))

    nation_supplier_partsupp_1 = nation_supplier_partsupp_0.sum(
        lambda x: ({x[0]: x[1]}) if (tmp_calc_value < ((x[0].ps_supplycost) * (x[0].ps_availqty))) else (None))
    ...
    nation_supplier_partsupp_3 = nation_supplier_partsupp_2.sum(
...
def q12(ord, li):
    ...
    urgent1 = "1-URGENT"
    high2 = "2-HIGH"
    orders_lineitem_probe = li.sum(lambda x: ({x[0]: x[1]}) if ((((((((
                (((x[0].l_shipmode == ship) + (x[0].l_shipmode == mail))) * (
                    x[0].l_commitdate < x[0].l_receiptdate))) * (x[0].l_shipdate < x[0].l_commitdate))) * (
                                                                             ...
                                                                             x[0].l_receiptdate < 19950101))) else (
        None))

    orders_lineitem_build_nest_dict = ord.sum(lambda x: {x[0].o_orderkey: sr_dict({x[0]: x[1]})})

    orders_lineitem_0 = orders_lineitem_probe.sum(
        lambda x: (orders_lineitem_build_nest_dict[x[0].l_orderkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (orders_lineitem_build_nest_dict[x[0].l_orderkey] != None) else (None))
...
def q13(cu, ord):
    special = "special"
    requests = "requests"
    orders_customer_index = ord.sum(lambda x: ({x[0]: x[1]}) if (((firstIndex(x[0].o_comment, special) != -1) * (
    ...
    orders_customer_1 = orders_customer_0.sum(
        lambda x: {record({"c_custkey": x[0].c_custkey}): record({"c_count": 1.0})})
...
def q14(li, pa):
    ...
    part_lineitem_0 = part_lineitem_probe.sum(
...
def q15(li, su):
    supplier_lineitem_index = su.sum(lambda x: {record(
        {"s_suppkey": x[0].s_suppkey, "s_name": x[0].s_name, "s_address": x[0].s_address,
         "s_phone": x[0].s_phone}): True})
    ...
    supplier_lineitem_probe = lineitem_3.sum(
        lambda x: ({x[0]: x[1]}) if (x[0].total_revenue == 1772627.2087) else (None))

    supplier_lineitem_build_nest_dict = supplier_lineitem_index.sum(lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})

    supplier_lineitem_0 = supplier_lineitem_probe.sum(
...
def q16(ps, pa, su):
    ...
    part_partsupp_index = part_0.sum(lambda x: {record(
...
def q17(li, pa):
    ...
    part_lineitem_lineitem_index = part_lineitem_probe.sum(
        lambda x: (part_lineitem_build_nest_dict[x[0].l_partkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (part_lineitem_build_nest_dict[x[0].l_partkey] != None) else (None))

    part_lineitem_lineitem_build_nest_dict = part_lineitem_lineitem_index.sum(
        lambda x: {x[0].l_partkey: sr_dict({x[0]: x[1]})})

    part_lineitem_lineitem_0 = li.sum(
        lambda x: (part_lineitem_lineitem_build_nest_dict[x[0].l_partkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (part_lineitem_lineitem_build_nest_dict[x[0].l_partkey] != None) else (None))

    part_lineitem_lineitem_1 = part_lineitem_lineitem_0.sum(lambda x: {x[0].concat(record({"l_extendedprice": (
        x[0].l_extendedprice) if (x[0].l_quantity < ((0.2) * (((x[0].sum_quant) / (x[0].count_quant))))) else (0.0)})):
                                                                           x[1]})
...
def q18(li, cu, ord):
    ...
    customer_orders_0 = customer_orders_probe.sum(
        lambda x: (customer_orders_build_nest_dict[x[0].o_custkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (customer_orders_build_nest_dict[x[0].o_custkey] != None) else (None))

    customer_orders_lineitem_index = customer_orders_0.sum(lambda x: {record(
    ...
    customer_orders_lineitem_0 = li.sum(
        lambda x: (customer_orders_lineitem_build_nest_dict[x[0].l_orderkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (customer_orders_lineitem_build_nest_dict[x[0].l_orderkey] != None) else (None))

    customer_orders_lineitem_1 = customer_orders_lineitem_0.sum(lambda x: {record(
        {"c_name": x[0].c_name, "c_custkey": x[0].c_custkey, "o_orderkey": x[0].o_orderkey,
...
def q19(li, pa):
    ...
    part_lineitem_probe = li.sum(lambda x: ({x[0]: x[1]}) if (
...
def q20(su, na, ps, pa, li):
    ...
    lineitem_partsupp_build_nest_dict = lineitem_partsupp_index.sum(
        lambda x: {record({"l_partkey": x[0].l_partkey, "l_suppkey": x[0].l_suppkey}): sr_dict({x[0]: x[1]})})

    lineitem_partsupp_0 = ps.sum(lambda x: (
        lineitem_partsupp_build_nest_dict[record({"ps_partkey": x[0].ps_partkey, "ps_suppkey": x[0].ps_suppkey})].sum(
            lambda y: {x[0].concat(y[0]): True})
        ) if (lineitem_partsupp_build_nest_dict[
                  record({"ps_partkey": x[0].ps_partkey, "ps_suppkey": x[0].ps_suppkey})] != None) else (None))

    lineitem_partsupp_1 = lineitem_partsupp_0.sum(
...
def q21(su, li, ord, na):
    ...
    orders_nation_supplier_lineitem_lineitem_lineitem_index = ord.sum(
    ...
    nation_supplier_lineitem_lineitem_lineitem_index = su.sum(
        lambda x: (nation_supplier_build_nest_dict[x[0].s_nationkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (nation_supplier_build_nest_dict[x[0].s_nationkey] != None) else (None))
    ...
    lineitem_lineitem_lineitem_index = l3_2.sum(
    ...
    lineitem_lineitem_index = l2_1.sum(
    ...
    lineitem_lineitem_lineitem_probe = lineitem_lineitem_probe.sum(
        lambda x: (lineitem_lineitem_build_nest_dict[x[0].l_orderkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (lineitem_lineitem_build_nest_dict[x[0].l_orderkey] != None) else (None))

    lineitem_lineitem_lineitem_build_nest_dict = lineitem_lineitem_lineitem_index.sum(
        lambda x: {x[0].l_orderkey: sr_dict({x[0]: x[1]})})

    nation_supplier_lineitem_lineitem_lineitem_probe = lineitem_lineitem_lineitem_probe.sum(
        lambda x: (lineitem_lineitem_lineitem_build_nest_dict[x[0].l_orderkey].sum(lambda y: {x[0].concat(y[0]): True})
                   ) if (lineitem_lineitem_lineitem_build_nest_dict[x[0].l_orderkey] != None) else (None))

    nation_supplier_lineitem_lineitem_lineitem_build_nest_dict = nation_supplier_lineitem_lineitem_lineitem_index.sum(
        lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})

    orders_nation_supplier_lineitem_lineitem_lineitem_probe = nation_supplier_lineitem_lineitem_lineitem_probe.sum(
        lambda x: (nation_supplier_lineitem_lineitem_lineitem_build_nest_dict[x[0].l_suppkey].sum(
            lambda y: {x[0].concat(y[0]): True})
                   ) if (nation_supplier_lineitem_lineitem_lineitem_build_nest_dict[x[0].l_suppkey] != None) else (
            None))

    orders_nation_supplier_lineitem_lineitem_lineitem_build_nest_dict = orders_nation_supplier_lineitem_lineitem_lineitem_index.sum(
        lambda x: {x[0].o_orderkey: sr_dict({x[0]: x[1]})})

    orders_nation_supplier_lineitem_lineitem_lineitem_0 = orders_nation_supplier_lineitem_lineitem_lineitem_probe.sum(
        lambda x: (orders_nation_supplier_lineitem_lineitem_lineitem_build_nest_dict[x[0].l_orderkey].sum(
            lambda y: {x[0].concat(y[0]): True})
                   ) if (
                    orders_nation_supplier_lineitem_lineitem_lineitem_build_nest_dict[x[0].l_orderkey] != None) else (
            None))

    orders_nation_supplier_lineitem_lineitem_lineitem_1 = orders_nation_supplier_lineitem_lineitem_lineitem_0.sum(
        lambda x: ({x[0]: x[1]}) if (((x[0].l2_size > 1) * (x[0].l3_size == 1))) else (None))

    orders_nation_supplier_lineitem_lineitem_lineitem_2 = orders_nation_supplier_lineitem_lineitem_lineitem_1.sum(
        lambda x: {record({"s_name": x[0].s_name}): record({"numwait": 1.0})})
