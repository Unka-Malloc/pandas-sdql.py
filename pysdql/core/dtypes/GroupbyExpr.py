...
class GroupbyExpr:
    ...
    def gen_tmp_name(self, noname=None):
        ...
        dup_list = [self.name, self.groupby_from.field] + self.history_name + self.groupby_from.history_name + noname
    ...
    def get_nested_dict(self):
        ...
        tmp_key = RecEl(tmp_dict)
        tmp_val = DictEl({self.last_iter.key: self.last_iter.val})

        nested_dict = VarExpr(name=self.nested_dict_name,
                              data=IterStmt(self.last_iter, DictEl({tmp_key: tmp_val})))
    ...
    def get_grouped_dict(self):
        ...
        result = VarExpr(name=self.name,
                         data=IterStmt(self.nested_dict_iter_expr, DictEl({RecEl(tmp_dict): 1})))

        self.history_name.append(self.name)
        self.operations.append(OpExpr('groupby_grouped_dict', result))
    ...
    def optimized_agg_kwargs_parse(self, agg_dict):
        ...
        for k in agg_dict.keys():
            v = agg_dict[k]
            if type(v) == ColEl:
                if v.field in tmp_cols:
                    tmp_cols.remove(v.field)
        ...
        for aggr_key in agg_dict.keys():
            ...
            if type(aggr_val) == ColEl:
                result_dict[aggr_key] = f'{aggr_tuple_iter_expr.key}.{agg_dict[aggr_key].field}'
        ...
        next_name = f'agg_{self.groupby_from.field}'

        result = VarExpr(name=next_name,
                         data=IterStmt(aggr_tuple_iter_expr, DictEl({RecEl(result_dict): 1})))
    ...
    def aggregate_dict_parse(self, aggr_func: dict):
        ...
        next_name = f'aggr_{self.groupby_from.field}'
    ...
    def aggregate_kwargs_parse(self, aggr_dict: dict):
        ...
        for aggr_key in aggr_dict.keys():
            ...
            if type(aggr_val) == ColEl:
                result_dict[aggr_key] = f'{aggr_tuple_iter_expr.key}.{aggr_dict[aggr_key].field}'
            if type(aggr_val) == tuple:
    ...
    def filter(self, func):
        ...
        from pysdql import relation
        return relation(name=output.field,
                        inherit_from=self)
