...
from pysdql.core.exprs.basic import (
    ColEl,
)
...
class Retriever:
    ...
    def find_cols(expr_obj, as_expr=False) -> list:
        ...
        elif isinstance(expr_obj, ColOpBinary):
            cols += Retriever.find_cols(expr_obj.unit1, as_expr)
            cols += Retriever.find_cols(expr_obj.unit2, as_expr)
        elif isinstance(expr_obj, BinCondExpr):
            cols += Retriever.find_cols(expr_obj.unit1, as_expr)
            cols += Retriever.find_cols(expr_obj.unit2, as_expr)
        elif isinstance(expr_obj, ColOpExternal):
    ...
    def findall_col_rename(self, reverse=False) -> dict:
        ...
        for op_expr in self.history:
            ...
            # OldColOpExpr
            if isinstance(op_body, OldColRename):
                if reverse:
        ...
        for op_expr in self.history:
            ...
            if isinstance(op_body, OldColRename):
                ...
                if mode == 'as_val':
                    ...
                    elif isinstance(op_body.col_expr, (ColEl, ColOpBinary)):
    ...
    def findall_cols_used(self, as_owner=True, only_next=False, exclude='infer') -> list:
        ...
        for op_expr in self.history:
            ...
            if isinstance(op_body, NewColInsert):
                ...
                if isinstance(op_body.col_expr, (ColEl, ColOpBinary, ColOpExternal)):
                    cols_used += self.find_cols(op_body.col_expr)
                elif isinstance(op_body.col_expr, Expr):
                    cols_used += SDQLInspector.find_cols(op_body.col_expr)
    ...
    def findall_col_insert_as_list(self) -> dict:
        ...
        for op_expr in self.history:
            ...
            if isinstance(op_body, NewColListInsert):
    ...
    def last_iter_is_calc(self):
        return isinstance(self.find_last_iter(), AggrBinOp)
    ...
    def find_last_iter(self, body_only=True, as_enum=False):
        for op_expr in reversed(self.history):
            ...
            if isinstance(op_body, AggrBinOp):
                if as_enum:
    ...
    @staticmethod
    def split_cond(cond: BinCondExpr, mapper: dict) -> dict:
        on = {}

        return on

    @staticmethod
    def find_cond_on(cond: BinCondExpr, mapper: dict, as_expr=False) -> list:
        on = []
        ...
        if isinstance(cond.unit1, ColEl):
            ...
            for n in mapper.keys():
                if col_name in mapper[n]:
                    on.append(n)
        elif isinstance(cond.unit1, ColOpBinary):
            for col_name in Retriever.find_cols(cond.unit1):
                for n in mapper.keys():
                    if col_name in mapper[n]:
                        on.append(n)
        elif isinstance(cond.unit1, ColOpExternal):
            col_name = cond.unit1.col.field
            for n in mapper.keys():
                if col_name in mapper[n]:
                    on.append(n)
        elif isinstance(cond.unit1, BinCondExpr):
        ...
        if isinstance(cond.unit2, ColEl):
            ...
            for n in mapper.keys():
                if col_name in mapper[n]:
                    on.append(n)
        elif isinstance(cond.unit2, ColOpBinary):
            for col_name in Retriever.find_cols(cond.unit2):
                for n in mapper.keys():
                    if col_name in mapper[n]:
                        on.append(n)
        elif isinstance(cond.unit2, ColOpExternal):
            col_name = cond.unit2.col.field
            for n in mapper.keys():
                if col_name in mapper[n]:
                    on.append(n)
        elif isinstance(cond.unit2, BinCondExpr):
    ...
    def findall_cols_in_cond(cond, as_expr=False):
        ...
        elif isinstance(cond.unit1, (ColOpBinary, ColOpExternal)):
        ...
        elif isinstance(cond.unit2, (ColOpBinary, ColOpExternal)):
    ...
    @staticmethod
    def replace_cond(cond: BinCondExpr, mapper: dict) -> BinCondExpr:
        new_unit1 = cond.unit1
        new_unit2 = cond.unit2

        if isinstance(cond.unit1, BinCondExpr):
            new_unit1 = Retriever.replace_cond(cond.unit1, mapper)
        if isinstance(cond.unit2, BinCondExpr):
    ...
    def as_bypass_for_next_join(self):
        ...
        if self.as_part_for_next_join:
            if not self.find_ops_except([ColProj, MergeExpr]):
    ...
    def find_aggr_filt(self):
        for op_expr in self.history:
            ...
            if isinstance(op_body, AggrOpFilter):
                return op_body
    ...
    @staticmethod
    def equals(expr1: Replaceable, expr2: Replaceable) -> bool:
    ...
    def insert_aggr(self, to_which):
        ...
        for aggr_expr in aggr_list:
            ...
            if aggr_name not in col_ins_list:
                new_col_obj = NewColInsert(aggr_name, aggr_expr.aggr_op[aggr_name])
    ...
    def find_aggr_in_calc(target):
        ...
        if isinstance(target.unit1, AggrBinOp):
            col_ops += Retriever.find_aggr_in_calc(target.unit1)

        if isinstance(target.unit2, AggrBinOp):
    ...
    def purify_cond(self, target, accepted):
        if isinstance(target, BinCondExpr):
    ...
    def findall_additional_columns(self):
        ...
        for op_expr in self.history:
            ...
            if isinstance(op_body, ColProjExtra):
