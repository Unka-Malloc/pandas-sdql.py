...
from pysdql.core.exprs.advanced.AggrFuncExprs import (
    AggrNuniqueFunc,
)
        ...
        if self.retriever.is_last_joint():
                ...
                if len(aggr_dict.keys()) == 1:
                    ...
                    if isinstance(dict_val, RecAccessExpr):
                        ...
                        if any([k not in probe_on.columns for k in SDQLInspector.find_cols(dict_val)]):
                            mapper = {}
                            for c in SDQLInspector.find_cols(dict_val):
                                if c not in probe_on.columns:
                                    mapper[c] = self.retriever.find_lookup_path(self, c)
                            aggr_body = SDQLInspector.replace_field(dict_val, inplace=True, mapper=mapper)
                        else:
                            aggr_body = dict_val
            ...
            if self.retriever.last_iter_is_groupby_aggr:
                ...
                if self.probe_frame.retriever.is_joint:
                    ...
                    if self.part_frame.retriever.as_bypass_for_next_join:
                        ...
                        for k in self.groupby_cols:
                            ...
                            if k in root_part_side.columns:
                                ...
                                v = self.col_ins[k]
                                if isinstance(v, (ColEl, ColOpBinary)):
                                    col_op = v.field
                                    for this_part_side in all_part_sides:
                                        if col_op in this_part_side.columns:
                                            ...
                                            k_found = True
                                elif isinstance(v, ColOpExternal):
                                    col_op = v.col.field
                    ...
                    if self.part_frame.retriever.was_probed:
                        ...
                        # print(self.part_frame.part_key, self.probe_frame.probe_key)
                        ...
                        if isinstance(part_key, str) and isinstance(probe_key, str):
                            ...
                            if len(groupby_cols) == 0:
                                ...
                                for c in groupby_cols:
                                    if c == probe_key:
                                        key_tuples.append((c, RecConsExpr([self.probe_access(i) for i in probe_key])))
                                    elif c == part_key:
                                        key_tuples.append((c, RecConsExpr([self.probe_access(i) for i in probe_key])))
                                    elif c in probe_on.columns:
                                        key_tuples.append((c, self.probe_access(c)))
                            ...
                            # print('name', self.joint.name)
                            # print('expr', aggr_body)
                        ...
                        if len(aggr_dict.keys()) == 0:
                            ...
                            if isinstance(dict_val, RecAccessExpr):
                                ...
                                aggr_body = DicConsExpr([(dict_key_ir,
                                                          dict_val)])
                            elif isinstance(dict_val, AggrNuniqueFunc):
                                # x.nunique
                                nunique_column = True
                                aggr_body = DicConsExpr([(dict_key_ir,
                                    ...
                                    if v_name in probe_on.columns:
                                        ...
                                        if v_name in joint_col_ins.keys():
                                            ...
                                            if isinstance(new_col,
                                                          (ColEl, ColOpBinary, ColOpExternal, ColOpApply,
                                                           NewColInsert, OldColRename)):
                                                val_tuples.append((k,
            ...
            if self.retriever.last_iter_is_calc:
                ...
                for i in self.retriever.findall_col_insert().keys():
                    if isinstance(col_inserted[i], ColOpApply):
                        target = col_inserted[i].sdql_ir
                        if isinstance(target, IfExpr):
                            if any([k not in probe_on.columns for k in SDQLInspector.find_cols(target)]):
                                mapper = {}
                                for c in SDQLInspector.find_cols(target):
                                    if c not in probe_on.columns:
                                        mapper[c] = self.retriever.find_lookup_path(self, c)
                                new_op = SDQLInspector.replace_field(target, inplace=True, mapper=mapper)
                                rec_list.append((i, new_op))
                            else:
                                rec_list.append((i, target))
                        elif isinstance(target, (AddExpr, SubExpr, MulExpr, DivExpr)):
                            if any([k not in probe_on.columns for k in SDQLInspector.find_cols(target)]):
                                mapper = {}
                                for c in SDQLInspector.find_cols(target):
                                    if c not in probe_on.columns:
                                ...
                                rec_list.append((i, IfExpr(condExpr=self.part_nonull(),
                                                           thenBodyExpr=target,
                                                           elseBodyExpr=ConstantExpr(0.0))))
                    elif isinstance(col_inserted[i], Replaceable):
                ...
                for j in self.retriever.findall_col_rename().keys():
                    if isinstance(col_renamed[j], ColOpApply):
                        target = col_renamed[j].sdql_ir
                        if isinstance(target, IfExpr):
                            if any([k not in probe_on.columns for k in SDQLInspector.find_cols(target)]):
                                mapper = {}
                                for c in SDQLInspector.find_cols(target):
                            ...
                            if any([k not in probe_on.columns for k in SDQLInspector.find_cols(target)]):
                                ...
                                rec_list.append((j, IfExpr(condExpr=self.part_nonull(),
                                                           thenBodyExpr=target,
                                                           elseBodyExpr=ConstantExpr(0.0))))
                    elif isinstance(col_renamed[j], Replaceable):
            ...
            if self.retriever.as_part_for_next_join:
                if self.probe_frame.retriever.is_joint:
                    ...
                    for lookup_expr in SDQLInspector.findall_non_null(joint_op):
                        joint_op = IfExpr(
                            condExpr=CompareExpr(CompareSymbol.NE,
                                                 lookup_expr,
                                                 ConstantExpr(None)),
                            ...
                            for c in cols_not_in_root_probe:
                                for this_part_side in all_part_sides:
                                    if c in this_part_side.columns:
                                        ...
                                        cond_replace_mapper[c] = lookup_expr
