...
class record(sr_dict):
    ...
    def getContainer(self):
        return self._sr_dict__container

    def __repr__(self):
        # as <k = v>
        # return f"<{', '.join([f'{k} = {self._sr_dict__container}' for k in self._sr_dict__container.keys()])}>"

        # as [(k, v)]
        return f'{list(self._sr_dict__container.items())}'
...
def sdqlpy_init(execution_mode=0, threads_count=1):
    ...
    if execution_mode == 1:
        ...
        subprocess.call(current_dir + "/compiler.sh " + current_dir + " " + caller_filename + " " + str(threads_count),
                        shell=True)
...
def benchmark(title, iterations, func, args, show_results=True, verbose=True):
    ...
    if verbose:
        print(
            title + ": Mean: " + "{0:0.2f}".format(((totalTime * 1.0) / iterations)) + " | StDev: " + "{0:0.2f}".format(
                statistics.stdev(timesList)))
        if show_results:
