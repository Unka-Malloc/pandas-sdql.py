...
def query(ps, pa, su):
    ...
    partsupp_part_probe = pa.sum(lambda x: ({x[0]: x[1]}) if (((((((((((((((((((x[0].p_size == 9) + (x[0].p_size == 36))) + (x[0].p_size == 49))) + (x[0].p_size == 14))) + (x[0].p_size == 23))) + (x[0].p_size == 45))) + (x[0].p_size == 19))) + (x[0].p_size == 3))) * (x[0].p_brand != brand45))) * (startsWith(x[0].p_type, mediumpolished) == False))) else (None))
    ...
    supplier_partsupp_part_isin_build_index = supplier_0.sum(lambda x: {x[0].s_suppkey: True})
    ...
    partsupp_part_2 = partsupp_part_1.sum(lambda x: {record({"p_brand": x[0].p_brand, "p_type": x[0].p_type, "p_size": x[0].p_size}): record({"supplier_cnt": (1.0) if (x[0].ps_suppkey != None) else (0.0)})})
    
    results = partsupp_part_2.sum(lambda x: {x[0].concat(x[1]): True})
