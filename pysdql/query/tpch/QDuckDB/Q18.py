...
def query(li, cu, ord):
    ...
    lineitem_orders_customer_build_pre_ops = ord.sum(lambda x: (lineitem_orders_build_nest_dict[x[0].o_orderkey].sum(lambda y: {x[0].concat(y[0]): True})
    ...
    lineitem_lineitem_orders_customer_isin_pre_ops = lineitem_2.sum(lambda x: ({x[0]: x[1]}) if (x[0].suml_quantity > 300.0) else (None))
