...
def query(li, cu, ord, re, na, su):
    ...
    lineitem_supplier_nation_region_orders_customer_build_nest_dict = lineitem_supplier_nation_region_orders_customer_index.sum(lambda x: {record({"n_nationkey": x[0].n_nationkey, "s_nationkey": x[0].s_nationkey, "o_custkey": x[0].o_custkey}): sr_dict({x[0]: x[1]})})
