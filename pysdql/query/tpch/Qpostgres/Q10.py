...
def query(cu, ord, li, na):
    ...
    customer_lineitem_orders = ord.sum(lambda x_orders: (((({customer_part[x_orders[0].o_custkey].c_nationkey: record({"c_custkey": customer_part[x_orders[0].o_custkey].c_custkey, "c_name": customer_part[x_orders[0].o_custkey].c_name, "c_acctbal": customer_part[x_orders[0].o_custkey].c_acctbal, "c_address": customer_part[x_orders[0].o_custkey].c_address, "c_phone": customer_part[x_orders[0].o_custkey].c_phone, "c_comment": customer_part[x_orders[0].o_custkey].c_comment, "c_nationkey": customer_part[x_orders[0].o_custkey].c_nationkey, "l_extendedprice": lineitem_part[x_orders[0].o_orderkey].l_extendedprice, "l_discount": lineitem_part[x_orders[0].o_orderkey].l_discount})}) if (customer_part[x_orders[0].o_custkey] != None) else (None)) if (lineitem_part[x_orders[0].o_orderkey] != None) else (None)) if (lineitem_part[x_orders[0].o_orderkey] != None) else (None)) if (((x_orders[0].o_orderdate >= 19931001) * (x_orders[0].o_orderdate < 19940101))) else (None))
    
    nation_aggr = na.sum(lambda x_nation: ({record({"c_custkey": customer_lineitem_orders[x_nation[0].n_nationkey].c_custkey, "n_name": x_nation[0].n_name}): ((customer_lineitem_orders[x_nation[0].n_nationkey].l_extendedprice) * (((1) - (customer_lineitem_orders[x_nation[0].n_nationkey].l_discount))))}) if (customer_lineitem_orders[x_nation[0].n_nationkey] != None) else (None))
