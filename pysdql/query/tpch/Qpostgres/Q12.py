...
def query(ord, li):
    ...
    urgent1 = "1-URGENT"
    high2 = "2-HIGH"
    lineitem_0 = li.sum(lambda x: ({x[0]: x[1]}) if (((((((((((x[0].l_shipmode == ship) + (x[0].l_shipmode == mail))) * (x[0].l_commitdate < x[0].l_receiptdate))) * (x[0].l_shipdate < x[0].l_commitdate))) * (x[0].l_receiptdate >= 19940101))) * (x[0].l_receiptdate < 19950101))) else (None))
    
    lineitem_1 = lineitem_0.sum(lambda x: {record({"l_shipmode": x[0].l_shipmode, "l_orderkey": x[0].l_orderkey}): True})
    
    orders_lineitem_probe_pre_ops = lineitem_0.sum(lambda x: {record({"l_shipmode": x[0].l_shipmode, "l_orderkey": x[0].l_orderkey}): True})
    ...
    orders_lineitem_1 = orders_lineitem_0.sum(lambda x: {record({"l_shipmode": x[0].l_shipmode, "o_orderpriority": x[0].o_orderpriority}): True})
    ...
    orders_lineitem_5 = orders_lineitem_4.sum(lambda x: {record({"high_line_count": x[0].high_line_count, "low_line_count": x[0].low_line_count}): True})
    
    results = orders_lineitem_5.sum(lambda x: {record({"high_line_count": x[0].high_line_count, "low_line_count": x[0].low_line_count}): True})
