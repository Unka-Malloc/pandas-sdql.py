...
def query(cu, ord):
    ...
    orders_customer_build_pre_ops = orders_0.sum(lambda x: {record({"o_orderkey": x[0].o_orderkey, "o_custkey": x[0].o_custkey}): True})
    ...
    orders_customer_1 = orders_customer_0.sum(lambda x: {record({"c_custkey": x[0].c_custkey, "o_orderkey": x[0].o_orderkey}): True})
