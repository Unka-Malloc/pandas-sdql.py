...
def query(li, pa):
    # Insert
    promo = "PROMO"
    lineitem_0 = li.sum(lambda x: ({x[0]: x[1]}) if (((x[0].l_shipdate >= 19950901) * (x[0].l_shipdate < 19951001))) else (None))
    
    lineitem_part_build_pre_ops = lineitem_0.sum(lambda x: {record({"l_extendedprice": x[0].l_extendedprice, "l_discount": x[0].l_discount, "l_partkey": x[0].l_partkey}): True})
    ...
    lineitem_part_1 = lineitem_part_0.sum(lambda x: {record({"p_type": x[0].p_type, "l_extendedprice": x[0].l_extendedprice, "l_discount": x[0].l_discount}): True})
