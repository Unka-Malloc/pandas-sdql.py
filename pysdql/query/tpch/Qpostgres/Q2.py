...
def query(pa, su, ps, ps1, na, re):
    ...
    part_partsupp_supplier_nation_region_build_pre_ops = part_0.sum(lambda x: {record({"p_partkey": x[0].p_partkey, "p_mfgr": x[0].p_mfgr}): True})
    ...
    supplier_nation_region_probe_pre_ops = nation_region_0.sum(lambda x: {record({"n_name": x[0].n_name, "n_nationkey": x[0].n_nationkey}): True})
    
    supplier_nation_region_build_nest_dict = su.sum(lambda x: {x[0].s_nationkey: sr_dict({x[0]: x[1]})})
    
    supplier_nation_region_0 = supplier_nation_region_probe_pre_ops.sum(lambda x: (supplier_nation_region_build_nest_dict[x[0].n_nationkey].sum(lambda y: {x[0].concat(y[0]): True})
    ) if (supplier_nation_region_build_nest_dict[x[0].n_nationkey] != None) else (None))
    
    partsupp_supplier_nation_region_probe_pre_ops = supplier_nation_region_0.sum(lambda x: {record({"s_acctbal": x[0].s_acctbal, "s_name": x[0].s_name, "s_address": x[0].s_address, "s_phone": x[0].s_phone, "s_comment": x[0].s_comment, "s_suppkey": x[0].s_suppkey, "n_name": x[0].n_name}): True})
    
    partsupp_supplier_nation_region_build_nest_dict = ps.sum(lambda x: {x[0].ps_suppkey: sr_dict({x[0]: x[1]})})
    
    partsupp_supplier_nation_region_0 = partsupp_supplier_nation_region_probe_pre_ops.sum(lambda x: (partsupp_supplier_nation_region_build_nest_dict[x[0].s_suppkey].sum(lambda y: {x[0].concat(y[0]): True})
    ) if (partsupp_supplier_nation_region_build_nest_dict[x[0].s_suppkey] != None) else (None))
    
    part_partsupp_supplier_nation_region_probe_pre_ops = partsupp_supplier_nation_region_0.sum(lambda x: {record({"s_acctbal": x[0].s_acctbal, "s_name": x[0].s_name, "s_address": x[0].s_address, "s_phone": x[0].s_phone, "s_comment": x[0].s_comment, "ps_partkey": x[0].ps_partkey, "ps_supplycost": x[0].ps_supplycost, "n_name": x[0].n_name}): True})
    ...
    part_partsupp_supplier_nation_region_0 = part_partsupp_supplier_nation_region_probe_pre_ops.sum(lambda x: (part_partsupp_supplier_nation_region_build_nest_dict[x[0].ps_partkey].sum(lambda y: {x[0].concat(y[0]): True})
    ) if (part_partsupp_supplier_nation_region_build_nest_dict[x[0].ps_partkey] != None) else (None))
    
    part_partsupp_supplier_nation_region_supplier_partsupp_part_nation_region_build_pre_ops = part_partsupp_supplier_nation_region_0.sum(lambda x: {record({"s_acctbal": x[0].s_acctbal, "s_name": x[0].s_name, "n_name": x[0].n_name, "p_partkey": x[0].p_partkey, "p_mfgr": x[0].p_mfgr, "s_address": x[0].s_address, "s_phone": x[0].s_phone, "s_comment": x[0].s_comment, "ps_supplycost": x[0].ps_supplycost}): True})
    ...
    supplier_partsupp_part_build_pre_ops = supplier_0.sum(lambda x: {record({"s_suppkey": x[0].s_suppkey, "s_name": x[0].s_name, "s_address": x[0].s_address, "s_nationkey": x[0].s_nationkey, "s_phone": x[0].s_phone, "s_acctbal": x[0].s_acctbal, "s_comment": x[0].s_comment}): True})
    ...
    partsupp_part_build_pre_ops = partsupp_0.sum(lambda x: {record({"ps_supplycost": x[0].ps_supplycost, "ps_suppkey": x[0].ps_suppkey, "ps_partkey": x[0].ps_partkey}): True})
    
    partsupp_part_build_nest_dict = partsupp_part_build_pre_ops.sum(lambda x: {x[0].ps_partkey: sr_dict({x[0]: x[1]})})
    
    partsupp_part_0 = pa.sum(lambda x: (partsupp_part_build_nest_dict[x[0].p_partkey].sum(lambda y: {x[0].concat(y[0]): True})
    ) if (partsupp_part_build_nest_dict[x[0].p_partkey] != None) else (None))
    
    supplier_partsupp_part_probe_pre_ops = partsupp_part_0.sum(lambda x: {record({"ps_supplycost": x[0].ps_supplycost, "ps_suppkey": x[0].ps_suppkey, "p_partkey": x[0].p_partkey}): True})
    
    supplier_partsupp_part_build_nest_dict = supplier_partsupp_part_build_pre_ops.sum(lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})
    
    supplier_partsupp_part_0 = supplier_partsupp_part_probe_pre_ops.sum(lambda x: (supplier_partsupp_part_build_nest_dict[x[0].ps_suppkey].sum(lambda y: {x[0].concat(y[0]): True})
    ) if (supplier_partsupp_part_build_nest_dict[x[0].ps_suppkey] != None) else (None))
    
    supplier_partsupp_part_nation_region_build_pre_ops = supplier_partsupp_part_0.sum(lambda x: {record({"ps_supplycost": x[0].ps_supplycost, "s_nationkey": x[0].s_nationkey, "p_partkey": x[0].p_partkey}): True})
    ...
    nation_region_build_pre_ops = nation_0.sum(lambda x: {record({"n_nationkey": x[0].n_nationkey, "n_name": x[0].n_name, "n_regionkey": x[0].n_regionkey, "n_comment": x[0].n_comment}): True})
    
    region_0 = re.sum(lambda x: ({x[0]: x[1]}) if (x[0].r_name == europe) else (None))
    
    nation_region_probe_pre_ops = region_0.sum(lambda x: {record({"r_regionkey": x[0].r_regionkey}): True})
    
    nation_region_build_nest_dict = nation_region_build_pre_ops.sum(lambda x: {x[0].n_regionkey: sr_dict({x[0]: x[1]})})
    ...
    supplier_partsupp_part_nation_region_1 = supplier_partsupp_part_nation_region_0.sum(lambda x: {record({"ps_supplycost": x[0].ps_supplycost, "p_partkey": x[0].p_partkey, "p_partkey": x[0].p_partkey}): True})
    
    supplier_partsupp_part_nation_region_2 = supplier_partsupp_part_nation_region_1.sum(lambda x: {record({"p_partkey": x[0].p_partkey}): record({"min_ps_supplycost": x[0].ps_supplycost})})
    
    supplier_partsupp_part_nation_region_3 = supplier_partsupp_part_nation_region_2.sum(lambda x: {x[0].concat(x[1]): True})
    
    supplier_partsupp_part_nation_region_4 = supplier_partsupp_part_nation_region_3.sum(lambda x: {x[0].concat(record({"minps_supplycost": x[0].min_ps_supplycost})): x[1]})
    
    supplier_partsupp_part_nation_region_5 = supplier_partsupp_part_nation_region_4.sum(lambda x: {record({"minps_supplycost": x[0].minps_supplycost, "p_partkey": x[0].p_partkey}): True})
