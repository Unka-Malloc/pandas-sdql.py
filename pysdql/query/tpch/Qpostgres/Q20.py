...
def query(su, na, ps, pa, li):
    ...
    supplier_nation_1 = supplier_nation_0.sum(lambda x: {record({"s_name": x[0].s_name, "s_address": x[0].s_address, "s_suppkey": x[0].s_suppkey}): True})
    
    lineitem_0 = li.sum(lambda x: ({x[0]: x[1]}) if (((x[0].l_shipdate >= 19940101) * (x[0].l_shipdate < 19950101))) else (None))
    
    lineitem_partsupp_build_pre_ops = lineitem_0.sum(lambda x: {record({"l_orderkey": x[0].l_orderkey, "l_partkey": x[0].l_partkey, "l_suppkey": x[0].l_suppkey, "l_linenumber": x[0].l_linenumber, "l_quantity": x[0].l_quantity, "l_extendedprice": x[0].l_extendedprice, "l_discount": x[0].l_discount, "l_tax": x[0].l_tax, "l_returnflag": x[0].l_returnflag, "l_linestatus": x[0].l_linestatus, "l_shipdate": x[0].l_shipdate, "l_commitdate": x[0].l_commitdate, "l_receiptdate": x[0].l_receiptdate, "l_shipinstruct": x[0].l_shipinstruct, "l_shipmode": x[0].l_shipmode, "l_comment": x[0].l_comment}): True})
    ...
    lineitem_partsupp_probe_pre_ops = ps.sum(lambda x: {record({"ps_partkey": x[0].ps_partkey, "ps_suppkey": x[0].ps_suppkey}): True})
    ...
    lineitem_partsupp_0 = lineitem_partsupp_probe_pre_ops.sum(lambda x: (lineitem_partsupp_build_nest_dict[record({"l_partkey": x[0].ps_partkey, "l_suppkey": x[0].ps_suppkey})].sum(lambda y: {x[0].concat(y[0]): True})
    ) if (lineitem_partsupp_build_nest_dict[record({"l_partkey": x[0].ps_partkey, "l_suppkey": x[0].ps_suppkey})] != None) else (None))
    
    lineitem_partsupp_1 = lineitem_partsupp_0.sum(lambda x: {record({"l_orderkey": x[0].l_orderkey, "l_partkey": x[0].l_partkey, "l_suppkey": x[0].l_suppkey, "l_linenumber": x[0].l_linenumber, "l_quantity": x[0].l_quantity, "l_extendedprice": x[0].l_extendedprice, "l_discount": x[0].l_discount, "l_tax": x[0].l_tax, "l_returnflag": x[0].l_returnflag, "l_linestatus": x[0].l_linestatus, "l_shipdate": x[0].l_shipdate, "l_commitdate": x[0].l_commitdate, "l_receiptdate": x[0].l_receiptdate, "l_shipinstruct": x[0].l_shipinstruct, "l_shipmode": x[0].l_shipmode, "l_comment": x[0].l_comment, "ps_partkey": x[0].ps_partkey, "ps_suppkey": x[0].ps_suppkey}): True})
    ...
    partsupp_lineitem_partsupp_probe_pre_ops = lineitem_partsupp_3.sum(lambda x: {record({"suml_quantity": x[0].suml_quantity}): True})
    ...
    partsupp_lineitem_partsupp_1 = partsupp_lineitem_partsupp_0.sum(lambda x: ({x[0]: x[1]}) if (x[0].ps_availqty > x[0].suml_quantity) else (None))
    
    partsupp_lineitem_partsupp_part_build_pre_ops = partsupp_lineitem_partsupp_1.sum(lambda x: {record({"ps_partkey": x[0].ps_partkey, "ps_suppkey": x[0].ps_suppkey, "ps_availqty": x[0].ps_availqty, "ps_supplycost": x[0].ps_supplycost, "ps_comment": x[0].ps_comment}): True})
    
    part_0 = pa.sum(lambda x: ({x[0]: x[1]}) if (startsWith(x[0].p_name, forest)) else (None))
    
    partsupp_lineitem_partsupp_part_probe_pre_ops = part_0.sum(lambda x: {record({"p_partkey": x[0].p_partkey, "p_name": x[0].p_name, "p_mfgr": x[0].p_mfgr, "p_brand": x[0].p_brand, "p_type": x[0].p_type, "p_size": x[0].p_size, "p_container": x[0].p_container, "p_retailprice": x[0].p_retailprice, "p_comment": x[0].p_comment}): True})
    ...
    partsupp_lineitem_partsupp_part_0 = partsupp_lineitem_partsupp_part_probe_pre_ops.sum(lambda x: (partsupp_lineitem_partsupp_part_build_nest_dict[x[0].p_partkey].sum(lambda y: {x[0].concat(y[0]): True})
    ) if (partsupp_lineitem_partsupp_part_build_nest_dict[x[0].p_partkey] != None) else (None))
    
    partsupp_lineitem_partsupp_part_supplier_nation_isin_pre_ops = partsupp_lineitem_partsupp_part_0.sum(lambda x: {record({"ps_suppkey": x[0].ps_suppkey}): True})
