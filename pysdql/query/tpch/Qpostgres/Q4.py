...
def query(ord, li):
    ...
    lineitem_aggr = li.sum(lambda x_lineitem: (({orders_part[x_lineitem[0].l_orderkey].o_orderpriority: 1})
                                               if (orders_part[x_lineitem[0].l_orderkey] != None) else (None)) if (x_lineitem[0].l_commitdate < x_lineitem[0].l_receiptdate) else (None))
