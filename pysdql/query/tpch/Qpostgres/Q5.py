...
def query(li, cu, ord, re, na, su):
    ...
    lineitem_supplier_nation_region_probe_pre_ops = supplier_nation_region_0.sum(lambda x: {record({"s_suppkey": x[0].s_suppkey, "s_nationkey": x[0].s_nationkey, "n_name": x[0].n_name, "n_nationkey": x[0].n_nationkey}): True})
    
    lineitem_supplier_nation_region_build_nest_dict = li.sum(lambda x: {x[0].l_suppkey: sr_dict({x[0]: x[1]})})
    
    lineitem_supplier_nation_region_0 = lineitem_supplier_nation_region_probe_pre_ops.sum(lambda x: (lineitem_supplier_nation_region_build_nest_dict[x[0].s_suppkey].sum(lambda y: {x[0].concat(y[0]): True})
    ) if (lineitem_supplier_nation_region_build_nest_dict[x[0].s_suppkey] != None) else (None))
    
    lineitem_supplier_nation_region_orders_build_pre_ops = lineitem_supplier_nation_region_0.sum(lambda x: {record({"l_extendedprice": x[0].l_extendedprice, "l_discount": x[0].l_discount, "l_orderkey": x[0].l_orderkey, "s_nationkey": x[0].s_nationkey, "n_name": x[0].n_name, "n_nationkey": x[0].n_nationkey}): True})
    
    orders_0 = ord.sum(lambda x: ({x[0]: x[1]}) if (((x[0].o_orderdate >= 19940101) * (x[0].o_orderdate < 19950101))) else (None))
    
    lineitem_supplier_nation_region_orders_probe_pre_ops = orders_0.sum(lambda x: {record({"o_custkey": x[0].o_custkey, "o_orderkey": x[0].o_orderkey}): True})
    ...
    lineitem_supplier_nation_region_orders_0 = lineitem_supplier_nation_region_orders_probe_pre_ops.sum(lambda x: (lineitem_supplier_nation_region_orders_build_nest_dict[x[0].o_orderkey].sum(lambda y: {x[0].concat(y[0]): True})
    ) if (lineitem_supplier_nation_region_orders_build_nest_dict[x[0].o_orderkey] != None) else (None))
    
    lineitem_supplier_nation_region_orders_customer_build_pre_ops = lineitem_supplier_nation_region_orders_0.sum(lambda x: {record({"o_custkey": x[0].o_custkey, "l_extendedprice": x[0].l_extendedprice, "l_discount": x[0].l_discount, "s_nationkey": x[0].s_nationkey, "n_name": x[0].n_name, "n_nationkey": x[0].n_nationkey}): True})
    ...
    lineitem_supplier_nation_region_orders_customer_1 = lineitem_supplier_nation_region_orders_customer_0.sum(lambda x: {record({"n_name": x[0].n_name, "l_extendedprice": x[0].l_extendedprice, "l_discount": x[0].l_discount}): True})
