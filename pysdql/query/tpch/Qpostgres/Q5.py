...
def query(li, cu, ord, re, na, su):
    ...
    lineitem_supplier_nation_region = re.sum(lambda x_region: (({x_region[0].l_orderkey: record({"l_discount": lineitem_part[supplier_part[nation_part[x_region[0].r_regionkey].n_nationkey].s_suppkey].l_discount, "l_extendedprice": lineitem_part[supplier_part[nation_part[x_region[0].r_regionkey].n_nationkey].s_suppkey].l_extendedprice, "n_name": nation_part[x_region[0].r_regionkey].n_name, "s_nationkey": supplier_part[nation_part[x_region[0].r_regionkey].n_nationkey].s_nationkey})}) if (nation_part[x_region[0].r_regionkey] != None) else (None)) if (x_region[0].r_name == asia) else (None))
    ...
    customer_aggr = cu.sum(lambda x_customer: ({x_customer[0].n_name: ((x_customer[0].l_extendedprice) * (((1) - (x_customer[0].l_discount))))}) if (lineitem_supplier_nation_region_orders[record({"c_custkey": x_customer[0].c_custkey, "c_nationkey": x_customer[0].c_nationkey})] != None) else (None))
