...
def query(ps, su, na):
    ...
    nation_supplier_partsupp_index = su.sum(lambda x: (nation_supplier_build_nest_dict[x[0].s_nationkey].sum(lambda y: {x[0].concat(y[0]): True})
    ) if (nation_supplier_build_nest_dict[x[0].s_nationkey] != None) else (None))
    
    nation_supplier_partsupp_build_nest_dict = nation_supplier_partsupp_index.sum(lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})
    ...
    tmp_calc_value = nation_supplier_partsupp_0.sum(lambda x: ((((x[0].ps_supplycost) * (x[0].ps_availqty))) * (0.0001)))
    
    nation_supplier_partsupp_1 = nation_supplier_partsupp_0.sum(lambda x: ({x[0]: x[1]}) if (tmp_calc_value < ((x[0].ps_supplycost) * (x[0].ps_availqty))) else (None))
