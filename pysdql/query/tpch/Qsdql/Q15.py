...
def query(li, su):
    ...
    v2 = v1.sum(lambda x: ({record({"l_suppkey": x[0].l_suppkey}): record({"total_revenue": x[0].revenue})}) if (True) else (None))
    
    v3 = v2.sum(lambda x: ({x[0].concat(x[1]): True}) if (True) else (None))
    
    v4 = v3.sum(lambda x: ({x[0]: x[1]}) if (x[0].total_revenue == 797313.3838) else (None))
    
    supplier_lineitem_probe = v4
    supplier_lineitem_part = su
    build_side = supplier_lineitem_part.sum(lambda x: ({x[0].s_suppkey: sr_dict({x[0]: x[1]})}) if (True) else (None))
