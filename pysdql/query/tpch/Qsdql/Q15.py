...
def query(li, su):
    ...
    supplier_lineitem_1 = supplier_lineitem_0.sum(lambda x: {x[0]: {record({"s_suppkey": x[0].s_suppkey, "s_name": x[0].s_name, "s_address": x[0].s_address, "s_phone": x[0].s_phone, "total_revenue": x[0].total_revenue}): True}})
