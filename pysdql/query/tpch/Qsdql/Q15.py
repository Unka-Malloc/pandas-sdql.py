...
def query(li, su):
    ...
    # Insert
    supplier_lineitem_index = su.sum(lambda x: {record({"s_suppkey": x[0].s_suppkey, "s_name": x[0].s_name, "s_address": x[0].s_address, "s_phone": x[0].s_phone}): True})
    ...
    supplier_lineitem_probe = lineitem_3.sum(lambda x: ({x[0]: x[1]}) if (x[0].total_revenue == 1772627.2087) else (None))
    
    supplier_lineitem_build_nest_dict = supplier_lineitem_index.sum(lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})
    
    supplier_lineitem_0 = supplier_lineitem_probe.sum(lambda x: (supplier_lineitem_build_nest_dict[x[0].l_suppkey].sum(lambda y: {x[0].concat(y[0]): True})
