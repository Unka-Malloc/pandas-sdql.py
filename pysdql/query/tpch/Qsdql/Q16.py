...
def query(ps, pa, su):
    ...
    partsupp_supplier_isin_build = partsupp_supplier_isin_build.sum(lambda x: ({x[0].s_suppkey: True}) if (True) else (None))
    
    v0 = ps.sum(lambda x: (({x[0]: x[1]}) if (partsupp_supplier_isin_build[x[0].ps_suppkey] == None) else (None)) if (True) else (None))
    ...
    v1 = v0.sum(lambda x: ({record({"p_brand": x[0].p_brand, "p_type": x[0].p_type, "p_size": x[0].p_size}): record({"supplier_cnt": 1})}) if (True) else (None))
