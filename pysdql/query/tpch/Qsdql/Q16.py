...
def query(ps, pa, su):
    ...
    part_partsupp_1 = part_partsupp_0.sum(lambda x: {record({"p_brand": x[0].p_brand, "p_type": x[0].p_type, "p_size": x[0].p_size}): record({"supplier_cnt": (1.0) if (x[0].ps_suppkey != None) else (0.0)})})
