...
def query(li, l1, pa):
    ...
    v0 = l1.sum(lambda x: {record({"l_partkey": x[0].l_partkey}): record({"sum_quant": x[0].l_quantity, "count_quant": 1.0})})
    ...
    v1 = v0.sum(lambda x: {x[0].concat(record({"l_extendedprice": (x[0].l_extendedprice) if (x[0].l_quantity < ((0.2) * (((x[0].sum_quant) / (x[0].count_quant))))) else (0.0)})): x[1]})
