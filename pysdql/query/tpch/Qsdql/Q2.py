...
def query(pa, su, ps, ps1, na, re):
    ...
    build_side = region_nation_supplier_part.sum(lambda x: ({x[0].n_nationkey: sr_dict({x[0]: x[1]})}) if (True) else (None))
    ...
    build_side = region_nation_supplier_ps1_part.sum(lambda x: ({x[0].s_suppkey: sr_dict({x[0]: x[1]})}) if (True) else (None))
    ...
    v1 = v0.sum(lambda x: ({record({"ps_partkey": x[0].ps_partkey}): record({"ps_supplycost": x[0].ps_supplycost})}) if (True) else (None))
    
    v2 = v1.sum(lambda x: ({x[0].concat(x[1]): True}) if (True) else (None))
    
    v3 = v2.sum(lambda x: ({x[0].concat(record({"min_supplycost": x[0].ps_supplycost})): x[1]}) if (True) else (None))
    
    region_nation_supplier_ps1_part_partsupp_part = v3
    build_side = region_nation_supplier_ps1_part_partsupp_part.sum(lambda x: ({x[0].ps_partkey: sr_dict({x[0]: x[1]})}) if (True) else (None))
    ...
    build_side = region_nation_supplier_region_nation_supplier_ps1_part_partsupp_part.sum(lambda x: ({x[0].s_suppkey: sr_dict({x[0]: x[1]})}) if (True) else (None))
