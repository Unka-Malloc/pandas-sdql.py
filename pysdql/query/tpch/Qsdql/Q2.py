...
def query(pa, su, ps, na, re):
    ...
    region_nation_supplier_partsupp = ps.sum(lambda x_ps1: ({x_ps1[0].ps_partkey: record({"min_supplycost": x_ps1[0].ps_supplycost, "ps_partkey": x_ps1[0].ps_partkey, "ps_suppkey": x_ps1[0].ps_suppkey, "s_suppkey": x_ps1[0].ps_suppkey})}) if (region_nation_supplier[x_ps1[0].ps_suppkey] != None) else (None))
