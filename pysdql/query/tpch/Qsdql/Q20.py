...
def query(su, na, ps, pa, li):
    ...
    lineitem_part_isin_build = lineitem_part_isin_build.sum(lambda x: ({x[0].p_partkey: True}) if (True) else (None))
    
    v1 = v0.sum(lambda x: (({x[0]: x[1]}) if (lineitem_part_isin_build[x[0].l_partkey] != None) else (None)) if (True) else (None))
    
    lineitem_supplier_isin_build = su.sum(lambda x: ({x[0].s_suppkey: True}) if (True) else (None))
    
    v2 = v1.sum(lambda x: (({x[0]: x[1]}) if (lineitem_supplier_isin_build[x[0].l_suppkey] != None) else (None)) if (True) else (None))
    
    v3 = v2.sum(lambda x: ({record({"l_partkey": x[0].l_partkey, "l_suppkey": x[0].l_suppkey}): record({"sum_quantity": x[0].l_quantity})}) if (True) else (None))
    
    v4 = v3.sum(lambda x: ({x[0].concat(x[1]): True}) if (True) else (None))
    
    lineitem_partsupp_part = v4
    build_side = lineitem_partsupp_part.sum(lambda x: ({record({"l_partkey": x[0].l_partkey, "l_suppkey": x[0].l_suppkey}): sr_dict({x[0]: x[1]})}) if (True) else (None))
    ...
    supplier_lineitem_partsupp_isin_build = supplier_lineitem_partsupp_isin_build.sum(lambda x: ({x[0].l_suppkey: True}) if (True) else (None))
    
    v0 = su.sum(lambda x: (({x[0]: x[1]}) if (supplier_lineitem_partsupp_isin_build[x[0].s_suppkey] != None) else (None)) if (True) else (None))
