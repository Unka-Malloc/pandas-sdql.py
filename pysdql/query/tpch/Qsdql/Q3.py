...
def query(li, cu, ord):
    ...
    build_side = customer_orders_lineitem_part.sum(lambda x: ({x[0].o_orderkey: sr_dict({x[0]: x[1]})}) if (True) else (None))
    ...
    v2 = v1.sum(lambda x: ({record({"l_orderkey": x[0].l_orderkey, "o_orderdate": x[0].o_orderdate, "o_shippriority": x[0].o_shippriority}): record({"revenue": x[0].revenue})}) if (True) else (None))
