...
def query(li, cu, ord):
    ...
    customer_orders_lineitem_index = customer_orders_probe.sum(lambda x: (customer_orders_build_nest_dict[x[0].o_custkey].sum(lambda y: {x[0].concat(y[0]): True})
    ...
    customer_orders_lineitem_0 = customer_orders_lineitem_probe.sum(lambda x: (customer_orders_lineitem_build_nest_dict[x[0].l_orderkey].sum(lambda y: {x[0].concat(y[0]): True})
