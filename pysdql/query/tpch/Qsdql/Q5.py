...
def query(li, cu, ord, re, na, su):
    ...
    region_nation = na.sum(lambda x_nation: ({x_nation[0].n_nationkey: record({"n_name": x_nation[0].n_name, "n_nationkey": x_nation[0].n_nationkey})}) if (region_part[x_nation[0].n_regionkey] != None) else (None))
    
    region_nation_customer = cu.sum(lambda x_customer: ({x_customer[0].c_custkey: record({"c_custkey": x_customer[0].c_custkey, "c_nationkey": x_customer[0].c_nationkey, "n_name": region_nation[x_customer[0].c_nationkey].n_name})}) if (region_nation[x_customer[0].c_nationkey] != None) else (None))
    
    region_nation_customer_orders = ord.sum(lambda x_orders: (({x_orders[0].o_orderkey: record({"c_nationkey": region_nation_customer[x_orders[0].o_custkey].c_nationkey, "n_name": region_nation_customer[x_orders[0].o_custkey].n_name, "o_orderkey": x_orders[0].o_orderkey})}) if (region_nation_customer[x_orders[0].o_custkey] != None) else (None)) if (((x_orders[0].o_orderdate >= 19950101) * (x_orders[0].o_orderdate < 19960101))) else (None))
