...
def query(pa, su, li, ord, cu, na, n1, n2, re):
    ...
    v0 = n1.sum(lambda x: {x[0].concat(record({"n1_nationkey": x[0].n_nationkey})): x[1]})
    ...
    build_side = supplier_part_region_n1_customer_orders_lineitem_part.sum(lambda x: {x[0].s_suppkey: sr_dict({x[0]: x[1]})})
    ...
    v0 = n2.sum(lambda x: {x[0].concat(record({"n2_nationkey": x[0].n_nationkey})): x[1]})
    ...
    build_side = n2_supplier_part_region_n1_customer_orders_lineitem_part.sum(lambda x: {x[0].n2_nationkey: sr_dict({x[0]: x[1]})})
    ...
    v4 = v3.sum(lambda x: {x[0].concat(record({"volume_A": (((x[0].l_extendedprice) * (((1.0) - (x[0].l_discount))))) if (x[0].nation == brazil) else (0.0)})): x[1]})
