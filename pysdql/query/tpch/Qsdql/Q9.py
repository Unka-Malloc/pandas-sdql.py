...
def query(li, ord, na, su, pa, ps):
    ...
    nation_supplier_part_partsupp_orders_lineitem = li.sum(lambda x_lineitem: (({record({"nation": nation_supplier_part_partsupp[x_lineitem[0].l_suppkey].n_name, "o_year": extractYear(orders_part[x_lineitem[0].l_orderkey].o_orderdate)}): record({"sum_profit": ((((x_lineitem[0].l_extendedprice) * (((1.0) - (x_lineitem[0].l_discount))))) - (((nation_supplier_part_partsupp[x_lineitem[0].l_suppkey].ps_supplycost) * (x_lineitem[0].l_quantity))))})}) if (nation_supplier_part_partsupp[x_lineitem[0].l_suppkey] != None) else (None)) if (orders_part[x_lineitem[0].l_orderkey] != None) else (None))
