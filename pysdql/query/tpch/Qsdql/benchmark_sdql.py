...
def q1(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(1, tpch_q1(lineitem).to_sdqlir(optimize, verbose=False))
...
def q2(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(2, tpch_q2(part, supplier, partsupp, nation, region).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(part_data, supplier_data, partsupp_data, nation_data, region_data)
...
def q3(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(3, tpch_q3(lineitem, customer, orders).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(lineitem_data, customer_data, orders_data)
...
def q4(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(4, tpch_q4(orders, lineitem).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(orders_data, lineitem_data)
...
def q5(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(5, tpch_q5(lineitem, customer, orders, region, nation, supplier).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(lineitem_data, customer_data, orders_data, region_data, nation_data, supplier_data)
...
def q6(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(6, tpch_q6(lineitem).to_sdqlir(optimize, verbose=False))
...
def q7(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(7, tpch_q7(supplier, lineitem, orders, customer, nation).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(supplier_data, lineitem_data, orders_data, customer_data, nation_data)
...
def q8(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(8, tpch_q8(part, supplier, lineitem, orders, customer, nation, region).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(part_data, supplier_data, lineitem_data, orders_data, customer_data, nation_data, region_data)
...
def q9(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(9, tpch_q9(lineitem, orders, nation, supplier, part, partsupp).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(lineitem_data, orders_data, nation_data, supplier_data, part_data, partsupp_data)
...
def q10(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(10, tpch_q10(customer, orders, lineitem, nation).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(customer_data, orders_data, lineitem_data, nation_data)
...
def q11(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(11, tpch_q11(partsupp, supplier, nation).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(partsupp_data, supplier_data, nation_data)
...
def q12(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(12, tpch_q12(orders, lineitem).to_sdqlir(optimize, verbose=False))
...
def q13(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(13, tpch_q13(customer, orders).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(customer_data, orders_data)
...
def q14(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(14, tpch_q14(lineitem, part).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(lineitem_data, part_data)
...
def q15(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(15, tpch_q15(lineitem, supplier).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(lineitem_data, supplier_data)
...
def q16(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(16, tpch_q16(partsupp, part, supplier).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(partsupp_data, part_data, supplier_data)
...
def q17(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(17, tpch_q17(lineitem, part).to_sdqlir(optimize, verbose=False))
...
def q18(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(18, tpch_q18(lineitem, customer, orders).to_sdqlir(optimize, verbose=False))
...
def q19(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(19, tpch_q19(lineitem, part).to_sdqlir(optimize, verbose=False))
...
def q20(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(20, tpch_q20(supplier, nation, partsupp, part, lineitem).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(supplier_data, nation_data, partsupp_data, part_data, lineitem_data)
...
def q21(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(21, tpch_q21(supplier, lineitem, orders, nation).to_sdqlir(optimize, verbose=False))
    ...
    Q.query(supplier_data, lineitem_data, orders_data, nation_data)
...
def q22(execution_mode=0, threads_count=1, optimize=True):
    ...
    write_query(22, tpch_q22(customer, orders).to_sdqlir(optimize, verbose=False))
